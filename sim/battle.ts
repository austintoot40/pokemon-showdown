...
import { type NuzlockeAI, createNuzlockeAI } from './nuzlocke-ai';
...
export class Battle {
	...
	setPlayer(slot: SideID, options: PlayerOptions) {
		...
		if (options.isAI && side.isAI !== options.isAI) {
			...
			this.nuzlockeAIInstance = createNuzlockeAI(this.nuzlockeDifficulty, this);
			didSomething = true;
		}
	...
	nuzlockeAI(request: ChoiceRequest): string | false {
		return this.nuzlockeAIInstance?.decide(request) ?? false;
	}
