...
export class Battle {
	...
	aiChooseMove(request: ChoiceRequest, difficulty: string): string {
		...
		// Pre-compute damage context: roll damage for each move once to find highest.
		// Some moves are excluded from the "highest damage" competition (have their own scoring):
		// trapping moves, Rollout/Ice Ball, Future Sight/Doom Desire.
		const aiSpeed = this.aiGetBoostedStat(aiActive, 'spe');
		const oppSpeed = this.aiGetBoostedStat(opponent, 'spe');
		const aiFaster = aiSpeed > oppSpeed;

		interface DmgCtx { damage: number; kills: boolean; faster: boolean; excludedFromHighest: boolean; }
		const dmgCtx = new Map<number, DmgCtx>();
		let highestDamage = 0;
		let anyKills = false;

		for (let i = 0; i < moves.length; i++) {
			if (moves[i].disabled) continue;
			const move = this.dex.moves.get(moves[i].id);
			if (move.basePower === 0) continue;
			if (!this.dex.getImmunity(move.type, opponent)) continue;
			const faster = aiFaster || move.priority > 0;
			const {damage} = this.aiSimulateDamage(move, aiActive, opponent);
			...
			dmgCtx.set(i, {damage, kills, faster, excludedFromHighest});
		...
		for (let i = 0; i < moves.length; i++) {
			if (moves[i].disabled) continue;
			const ctx = dmgCtx.get(i) ?? null;
			const isHighestDamage = ctx !== null && !ctx.excludedFromHighest && (anyKills ? ctx.kills : ctx.damage >= highestDamage);
			const score = this.aiScoreMove(moves[i].id, aiActive, opponent, difficulty, ctx, isHighestDamage);
			if (score > bestScore) {
				bestScore = score;
				bestSlot = i + 1;
				tiedCount = 1;
			} else if (score === bestScore) {
				// Randomly replace with equal-scored move (reservoir sampling)
				tiedCount++;
	...
	aiConsiderVoluntarySwitch(request: ChoiceRequest, difficulty: string): string | null {
		...
		if (difficulty === 'smart') {
			...
			const usableMoves = aiActive.moveSlots.filter(slot => {
				...
				if (!this.dex.getImmunity(move.type, opponent)) return false;
				return this.dex.getEffectiveness(move.type, opponent) >= 0; // ≥1×
			});
			...
			// Run and Bun spec: only switch when ALL damage moves are immune
			// (practical equivalent of "score <= -5" in the additive system)
			const allDamageMovesImmune = aiActive.moveSlots.every(slot => {
				if (slot.disabled) return true;
				const move = this.dex.moves.get(slot.id);
				if (move.basePower === 0) return true; // status moves don't block switching
				return !this.dex.getImmunity(move.type, opponent); // immune
			});
			if (!allDamageMovesImmune) return null;

			// Must be above 50% HP
			if (aiActive.hp / aiActive.maxhp <= 0.5) return null;

			// Viable bench: faster + not OHKO'd, or slower + not 2HKO'd
			const oppSpd = this.aiGetBoostedStat(opponent, 'spe');
			candidates = aiBench.filter(p => {
				const benchSpd = this.aiGetBoostedStat(p, 'spe');
				const incomingDmg = this.aiMaxOpponentDamage(opponent, p);
				if (benchSpd > oppSpd && incomingDmg < p.hp) return true; // faster, not OHKO'd
				if (benchSpd <= oppSpd && incomingDmg * 2 < p.hp) return true; // slower, not 2HKO'd
				return false;
			});
		...
		const bestBenchScore = Math.max(...candidates.map(p => this.aiScoreSwitchTarget(p, opponent)));
	...
	aiHighestDamageBonus(difficulty: string): number {
		if (difficulty === 'game-accurate') return 6;
		return Math.random() < 0.8 ? 6 : 8;
	}

	aiSimulateDamage(move: Move, attacker: Pokemon, defender: Pokemon): {damage: number} {
		if (move.basePower === 0 || !this.dex.getImmunity(move.type, defender)) return {damage: 0};
	...
	aiMaxOpponentDamage(attacker: Pokemon, defender: Pokemon): number {
		let maxDamage = 0;
		for (const slot of attacker.moveSlots) {
			const m = this.dex.moves.get(slot.id);
			if (m.basePower === 0 || !this.dex.getImmunity(m.type, defender)) continue;
			const {damage} = this.aiSimulateDamage(m, attacker, defender);
			if (damage > maxDamage) maxDamage = damage;
		}
		return maxDamage;
	}

	aiHasMoveCategory(pokemon: Pokemon, category: 'Physical' | 'Special'): boolean {
	...
	aiIsIncapacitated(pokemon: Pokemon): boolean {
		return pokemon.status === 'slp' || pokemon.status === 'frz';
	}

	aiShouldRecover(attacker: Pokemon, defender: Pokemon, healFraction: number, difficulty: string): boolean {
		if (difficulty === 'game-accurate') return false;
		const hpFrac = attacker.hp / attacker.maxhp;
		if (difficulty === 'smart') return hpFrac < 0.5;

		// competitive: full Run and Bun spec
		if (attacker.status === 'tox') return false;
		const healAmount = attacker.maxhp * healFraction;
		const maxDamage = this.aiMaxOpponentDamage(defender, attacker);
		if (maxDamage >= healAmount) return false;

		const faster = this.aiGetBoostedStat(attacker, 'spe') > this.aiGetBoostedStat(defender, 'spe');
		if (faster) {
	...
	aiScoreMove(
		...
		defender: Pokemon,
		difficulty: string,
		ctx: {damage: number; kills: boolean; faster: boolean} | null,
		isHighestDamage: boolean
	): number {
		const move = this.dex.moves.get(moveId);

		if (move.basePower > 0) {
			if (!this.dex.getImmunity(move.type, defender)) return -20;
			if (ctx === null) return -20;

			// --- ROLLOUT / ICE BALL: always +7 ---
			if (moveId === 'rollout' || moveId === 'iceball') return 7;

			// --- TRAPPING MOVES: own +6/+8, not part of highest-damage competition ---
			if ((move as AnyObject).flags?.trap) {
				let score = this.aiHighestDamageBonus(difficulty);
				if (difficulty !== 'game-accurate' && ctx.kills) score += ctx.faster ? 6 : 3;
				return score;
			}

			// --- FUTURE SIGHT / DOOM DESIRE: fixed base, not highest-damage ---
			if (moveId === 'futuresight' || moveId === 'doomdesire') {
				const dies = this.aiMaxOpponentDamage(defender, attacker) >= attacker.hp;
				let score = (ctx.faster && dies) ? 8 : 6;
				if (difficulty !== 'game-accurate' && ctx.kills) score += ctx.faster ? 6 : 3;
				return score;
			}
			...
			if (moveId === 'meteorbeam') {
				if (attacker.item !== 'powerherb' as ID) return -20;
				let score = 9;
				if (difficulty !== 'game-accurate' && ctx.kills) score += ctx.faster ? 6 : 3;
				return score;
			}

			// --- FELL STINGER: massive bonus for killing while not at +6 ATK ---
			if (moveId === 'fellstinger' && difficulty !== 'game-accurate') {
				if (ctx.kills && (attacker.boosts.atk ?? 0) < 6) {
					return ctx.faster
						? (Math.random() < 0.8 ? 21 : 23)
						: (Math.random() < 0.8 ? 15 : 17);
				}
				// otherwise: fall through to normal damage scoring
			}

			// --- PURSUIT: bonus based on opponent HP and AI speed ---
			// Pursuit bonuses stack on top of normal damage scoring (per spec note)

			// --- SUCKER PUNCH: anti-spam ---
			if (moveId === 'suckerpunch' && attacker.lastMove?.id === 'suckerpunch') {
				if (Math.random() < 0.5) return -20;
			}

			// --- FAKE OUT: +9 on first turn out ---
			if (moveId === 'fakeout' && attacker.activeTurns <= 1 &&
				defender.ability !== 'shielddust' as ID && defender.ability !== 'innerfocus' as ID) {
				const killBonus = (difficulty !== 'game-accurate' && ctx.kills) ? (ctx.faster ? 6 : 3) : 0;
				return 9 + killBonus;
			}

			let score = isHighestDamage ? this.aiHighestDamageBonus(difficulty) : 0;

			// Kill bonus (smart + competitive)
			if (difficulty !== 'game-accurate' && ctx.kills) {
				score += ctx.faster ? 6 : 3;
			}

			// Priority bonus when dying and slower (competitive only)
			if (difficulty === 'competitive' && move.priority > 0 && !ctx.faster) {
				if (this.aiMaxOpponentDamage(defender, attacker) >= attacker.hp) {
					score += 11;
				}
			}

			// Acid Spray: additional +6 on top of any normal scoring (not game-accurate)
			if (moveId === 'acidspray' && difficulty !== 'game-accurate') score += 6;

			// Pursuit: bonus stacks on top of normal damage scoring
			if (moveId === 'pursuit' && difficulty !== 'game-accurate') {
				if (ctx.kills) {
					score += 10;
				} else {
					const oppHpFrac = defender.hp / defender.maxhp;
					if (oppHpFrac < 0.2) score += 10;
					else if (oppHpFrac < 0.4 && Math.random() < 0.5) score += 8;
				}
				if (ctx.faster) score += 3;
			}

			// Contrary: stat-dropping moves become setup moves when not killing and not highest damage.
			// No OHKO guard, no Unaware guard — per spec these bypass those checks.
			if (attacker.ability === 'contrary' as ID && !isHighestDamage && !ctx.kills && difficulty !== 'game-accurate') {
				const selfBoosts = (move as AnyObject).self?.boosts;
				if (selfBoosts) {
					const spaBoost = -(selfBoosts.spa ?? 0); // Contrary inverts the drop
					const atkBoost = -(selfBoosts.atk ?? 0);
					if (spaBoost >= 2) {
						// Nasty Plot equivalent
						let contraScore = 6;
						if (this.aiIsIncapacitated(defender)) {
							contraScore += 3;
						} else {
							const can3HKO = this.aiMaxOpponentDamage(defender, attacker) * 3 >= attacker.hp;
							if (!can3HKO) {
								contraScore += 1;
								if (ctx.faster) contraScore += 1;
							}
						}
						if ((attacker.boosts.spa ?? 0) >= 2) contraScore -= 1;
						return contraScore;
					} else if (atkBoost >= 1) {
						// Bulk Up equivalent
						let contraScore = 6;
						const can2HKO = this.aiMaxOpponentDamage(defender, attacker) * 2 >= attacker.hp;
						if (this.aiIsIncapacitated(defender)) contraScore += 3;
						else if (can2HKO && !ctx.faster) contraScore -= 5;
						return contraScore;
					}
				}
			}

			// Speed-reduction moves (Rock Tomb, Icy Wind, Mud Shot, etc.):
			// When NOT the highest damage move, replace 0 with a utility score.
			// Spec: +6 if AI is slower and target can't block drops, else +5.
			if (!isHighestDamage && difficulty !== 'game-accurate' && score === 0) {
				...
				if (hasSpeDown) {
					score = (!blocked && !ctx.faster) ? 6 : 5;
				} else {
					// ATK or SpAtk reduction moves (Skitter Smack, Trop Kick, etc.)
					...
					if (hasAtkDown || hasSpaDown) {
						...
						const targetHasSplit = this.aiHasMoveCategory(defender, relevantCategory);
						score = (!blocked && targetHasSplit) ? 6 : 5;
					}
				}
			}
		...
		return this.aiScoreStatusMove(move, attacker, defender, difficulty);
	}

	aiScoreStatusMove(move: Move, attacker: Pokemon, defender: Pokemon, difficulty: string): number {
		const moveId = move.id;
		const hpFrac = attacker.hp / attacker.maxhp;
		const faster = this.aiGetBoostedStat(attacker, 'spe') > this.aiGetBoostedStat(defender, 'spe');

		// Universal: don't re-apply an existing status condition
		if (move.status && defender.status) return -20;

		// --- STEALTH ROCK ---
		if (moveId === 'stealthrock') {
			if (this.sides[0].sideConditions['stealthrock']) return -20;
			const firstTurn = attacker.activeTurns <= 1;
			if (difficulty === 'game-accurate') return 6;
			if (difficulty === 'smart') return firstTurn ? 7 : 6;
			// competitive: +8 (25%) or +9 (75%) first turn; +6 (25%) or +7 (75%) after
		...
		// --- SPIKES ---
		if (moveId === 'spikes') {
			const spikes = this.sides[0].sideConditions['spikes'];
			if (spikes && ((spikes as AnyObject).layers ?? 1) >= 3) return -20;
			const alreadySet = spikes ? 1 : 0;
			const firstTurn = attacker.activeTurns <= 1;
			if (difficulty === 'game-accurate') return 6;
			if (difficulty === 'smart') return (firstTurn ? 7 : 6) - alreadySet;
			const base = Math.random() < 0.25 ? 8 : 9;
		...
		if (moveId === 'toxicspikes') {
			const tspikes = this.sides[0].sideConditions['toxicspikes'];
		...
		// --- PARALYSIS ---
		if (move.status === 'par') {
			if (defender.types.includes('Electric')) return -20;
			if (difficulty === 'game-accurate') return 6;
			const oppSpe = this.aiGetBoostedStat(defender, 'spe');
			const aiSpe = this.aiGetBoostedStat(attacker, 'spe');
			const playerFasterButSlowedByPar = oppSpe > aiSpe && Math.floor(oppSpe / 4) < aiSpe;
			const aiHasFlinchMove = attacker.moveSlots.some(slot => {
				const m = this.dex.moves.get(slot.id);
				return (m as AnyObject).secondary?.volatileStatus === 'flinch';
			});
			const playerHasVolatile = !!(defender.volatiles['confusion'] || defender.volatiles['attract']);
			let score = (playerFasterButSlowedByPar || aiHasFlinchMove || playerHasVolatile) ? 8 : 7;
			if (Math.random() < 0.5) score -= 1; // spec: additional -1 50% of the time, all difficulties
			return score;
		}

		// --- BURN (Will-o-Wisp and other burn moves) ---
		if (move.status === 'brn') {
			if (defender.types.includes('Fire')) return -20;
			if (difficulty === 'game-accurate') return 6;
			if (difficulty === 'smart') return this.aiHasMoveCategory(defender, 'Physical') ? 6 : 5;
			// competitive
			let score = 6;
			if (Math.random() < 0.37) {
				if (this.aiHasMoveCategory(defender, 'Physical')) score += 1;
				if (attacker.moveSlots.some(s => s.id === 'hex')) score += 1;
			}
			return score;
		}
		...
		if (move.status === 'psn' || move.status === 'tox') {
			...
			let score = 6;
			if (difficulty === 'competitive' && Math.random() < 0.38) {
				const aiHasHex = attacker.moveSlots.some(s => s.id === 'hex');
				const aiHasVenoshock = attacker.moveSlots.some(s => s.id === 'venoshock');
				const aiHasMerciless = attacker.ability === 'merciless' as ID;
				const playerHasDmg = this.aiHasMoveCategory(defender, 'Physical') || this.aiHasMoveCategory(defender, 'Special');
				if ((aiHasHex || aiHasVenoshock || aiHasMerciless) && !playerHasDmg) score += 2;
			}
			return score;
		}

		// --- SLEEP (including Yawn, which uses volatileStatus instead of status) ---
		if (move.status === 'slp' || moveId === 'yawn') {
			// Don't use if target already has status or is already yawning
			if (defender.status) return -20;
			if (defender.volatiles['yawn']) return -20;
			if (difficulty === 'game-accurate') return 6;
			if (difficulty === 'smart') return 7;
			// competitive
			let score = 6;
			if (Math.random() < 0.25) {
				score += 1; // can always sleep if we got this far (status check passed)
				const aiHasDreamEater = attacker.moveSlots.some(s => s.id === 'dreameater');
				const aiHasNightmare = attacker.moveSlots.some(s => s.id === 'nightmare');
				const playerHasSnore = defender.moveSlots.some(s => s.id === 'snore');
				const playerHasSleepTalk = defender.moveSlots.some(s => s.id === 'sleeptalk');
		...
		// These heal 67% in sun but only 50% otherwise. Must be handled before the general recovery block.
		if (moveId === 'morningsun' || moveId === 'synthesis' || moveId === 'moonlight') {
			if (hpFrac >= 1.0) return -20;
			if (hpFrac >= 0.85) return -6;
			if (this.field.isWeather(['sunnyday', 'desolateland'])) {
				const shouldRecover = this.aiShouldRecover(attacker, defender, 2 / 3, difficulty);
				if (shouldRecover) return 7;
			}
			// Sun not active (or shouldRecover returned false): recalculate with 50%
			return this.aiShouldRecover(attacker, defender, 0.5, difficulty) ? 7 : 5;
		}

		// --- RECOVERY (Recover, Roost, Slack Off, Soft-Boiled, Heal Order, Strength Sap, etc.) ---
		if (move.heal?.[0] && (move.heal[0] as number) > 0) {
			if (hpFrac >= 1.0) return -20;
			if (hpFrac >= 0.85) return -6;
			const healNumerator = move.heal[0] as number;
			const healDenominator = (move.heal[1] as number) || 2;
			const shouldRecover = this.aiShouldRecover(attacker, defender, healNumerator / healDenominator, difficulty);
			return shouldRecover ? 7 : 5;
		}
		...
		if (moveId === 'rest') {
			if (hpFrac >= 1.0) return -20;
			if (hpFrac >= 0.85) return -6;
			const shouldRecover = this.aiShouldRecover(attacker, defender, 1.0, difficulty);
			if (!shouldRecover) return 5;
			if (difficulty === 'competitive') {
				const hasSleepCure = (
					attacker.item === 'lumberry' as ID ||
					attacker.item === 'chestoberry' as ID ||
					attacker.moveSlots.some(s => s.id === 'sleeptalk' || s.id === 'snore') ||
					attacker.ability === 'shedskin' as ID ||
					attacker.ability === 'earlybird' as ID ||
					(attacker.ability === 'hydration' as ID && this.field.isWeather('raindance'))
				);
		...
		// --- PROTECT / KING'S SHIELD / VARIANTS ---
		if (moveId === 'protect' || moveId === 'kingsshield' || moveId === 'spikyshield' || moveId === 'banefulbunker') {
			if (difficulty === 'game-accurate') return 5;
			if (difficulty === 'smart') {
				const lastId = attacker.lastMove?.id ?? '';
				if (lastId === 'protect' || lastId === 'kingsshield' || lastId === 'spikyshield' || lastId === 'banefulbunker') return -20;
				return 6;
			}
			// competitive: full spec
			let score = 6;
			const aiDraining = ['psn', 'tox', 'brn'].includes(attacker.status || '') ||
				!!(attacker.volatiles['curse'] || attacker.volatiles['leechseed'] ||
				attacker.volatiles['attract'] || attacker.volatiles['perish2']);
			if (aiDraining) score -= 2;
			const playerDraining = ['psn', 'tox', 'brn'].includes(defender.status || '') ||
				!!(defender.volatiles['curse'] || defender.volatiles['leechseed']);
			if (playerDraining) score += 1;
			if (attacker.activeTurns <= 1) score -= 1;
			const lastId = attacker.lastMove?.id ?? '';
		...
		if (moveId === 'substitute') {
			if (hpFrac <= 0.5) return -20;
			if (defender.ability === 'infiltrator' as ID) return -20;
			if (difficulty === 'game-accurate') return 5;
			if (difficulty === 'smart') return 6;
			// competitive
			let score = 6;
			if (defender.status === 'slp') score += 2;
			if (defender.volatiles['leechseed'] && faster) score += 2;
			if (Math.random() < 0.5) score -= 1;
			const playerHasSoundMove = defender.moveSlots.some(slot =>
				(this.dex.moves.get(slot.id) as AnyObject).flags?.sound
			);
		...
		if (moveId === 'taunt') {
			if (difficulty === 'competitive') {
				const trActive = !!this.field.pseudoWeather['trickroom'];
				const opponentHasTR = defender.moveSlots.some(s => s.id === 'trickroom');
				if (opponentHasTR && !trActive) return 9;
				const opponentHasDefog = defender.moveSlots.some(s => s.id === 'defog');
				const auroraVeilUp = !!this.sides[1].sideConditions['auroraveil'];
				if (opponentHasDefog && auroraVeilUp && faster) return 9;
				return 5;
			}
			const opponentHasStatus = defender.moveSlots.some(s => this.dex.moves.get(s.id).category === 'Status');
			return opponentHasStatus ? 6 : 5;
		}

		// --- ENCORE ---
		if (moveId === 'encore') {
			if (!defender.lastMove) return -20;
			if (defender.volatiles['encore']) return -20;
			if (difficulty === 'game-accurate') return 5;
			const lastMoveWasStatus = this.dex.moves.get(defender.lastMove.id).category === 'Status';
			if (lastMoveWasStatus) return faster ? 7 : (Math.random() < 0.5 ? 6 : 5);
			return Math.random() < 0.5 ? 6 : 5;
		}

		// --- EXPLOSION / SELF-DESTRUCT ---
		if (moveId === 'explosion' || moveId === 'selfdestruct' || moveId === 'mistyexplosion') {
			if (difficulty === 'game-accurate') return -20;
			if (difficulty === 'smart') return hpFrac < 0.2 ? 7 : 5;
			// competitive
			const aiAlive = this.sides[1].pokemon.filter(p => !p.fainted && p.hp > 0).length;
			const oppAlive = this.sides[0].pokemon.filter(p => !p.fainted && p.hp > 0).length;
			if (aiAlive === 1 && oppAlive > 1) return -20;
			let score: number;
			if (hpFrac < 0.1) score = 10;
			else if (hpFrac < 0.33) score = Math.random() < 0.7 ? 8 : 0;
			else if (hpFrac < 0.66) score = Math.random() < 0.5 ? 7 : 0;
		...
		// --- BATON PASS ---
		if (moveId === 'batonpass') {
			if (difficulty !== 'competitive') return 5;
			const bench = this.sides[1].pokemon.slice(1).filter(p => p && !p.fainted && p.hp > 0);
			if (bench.length === 0) return -20;
			const hasSub = !!attacker.volatiles['substitute'];
			const hasBoost = Object.values(attacker.boosts).some(v => (v as number) > 0);
		...
		if (moveId === 'destinybond') {
			if (difficulty !== 'competitive') return 5;
			const dies = this.aiMaxOpponentDamage(defender, attacker) >= attacker.hp;
			if (faster && dies) return Math.random() < 0.81 ? 7 : 6;
		...
		// --- IMPRISON ---
		if (moveId === 'imprison') {
			if (difficulty !== 'competitive') return 6;
			const hasSharedMove = defender.moveSlots.some(defSlot =>
				attacker.moveSlots.some(attSlot => attSlot.id === defSlot.id)
			);
		...
		// --- STICKY WEB ---
		if (moveId === 'stickyweb') {
			if (this.sides[0].sideConditions['stickyweb']) return -20;
			const firstTurn = attacker.activeTurns <= 1;
			if (difficulty === 'game-accurate') return 6;
			if (difficulty === 'smart') return firstTurn ? 9 : 6;
			// competitive: +9 (25%) or +12 (75%) first turn; +6 (25%) or +9 (75%) after
			return firstTurn
		...
		// --- TRICK ROOM ---
		if (moveId === 'trickroom') {
			if (this.field.pseudoWeather['trickroom']) return -20; // already active
			if (difficulty === 'game-accurate') return 6;
			return !faster ? 10 : 5;
		}

		// --- REFLECT / LIGHT SCREEN ---
		if (moveId === 'reflect' || moveId === 'lightscreen') {
			if (difficulty === 'game-accurate') return 6;
			const correspondingCategory: 'Physical' | 'Special' = moveId === 'reflect' ? 'Physical' : 'Special';
			let score = 6;
			if (this.aiHasMoveCategory(defender, correspondingCategory)) {
				if (attacker.item === 'lightclay' as ID) score += 1;
				if (Math.random() < 0.5) score += 1;
			}
			return score;
		}
		...
		// --- FINAL GAMBIT ---
		if (moveId === 'finalgambit') {
			if (difficulty === 'game-accurate') return 6;
			if (faster && attacker.hp > defender.hp) return 8;
			const dies = this.aiMaxOpponentDamage(defender, attacker) >= attacker.hp;
			if (faster && dies) return 7;
			return 6;
		}
		...
		if (moveId === 'memento') {
			if (difficulty !== 'competitive') return 6;
			const bench = this.sides[1].pokemon.slice(1).filter(p => !p.fainted && p.hp > 0);
			if (bench.length === 0) return -20; // never on last mon
			if (hpFrac < 0.1) return 16;
			if (hpFrac < 0.33) return Math.random() < 0.7 ? 14 : 6;
			if (hpFrac < 0.66) return Math.random() < 0.5 ? 13 : 6;
			return Math.random() < 0.05 ? 13 : 6;
		}

		// --- FOCUS ENERGY / LASER FOCUS ---
		if (moveId === 'focusenergy' || moveId === 'laserfocus') {
			if (defender.ability === 'shellarmor' as ID || defender.ability === 'battlearmor' as ID) return -20;
			if (difficulty === 'game-accurate') return 6;
			const hasHighCrit = (
				attacker.ability === 'superluck' as ID ||
				attacker.ability === 'sniper' as ID ||
				attacker.item === 'scopelens' as ID ||
				attacker.item === 'razorclaw' as ID ||
				attacker.moveSlots.some(s => ((this.dex.moves.get(s.id) as AnyObject).critRatio ?? 1) >= 2)
			);
		...
		if (['electricterrain', 'psychicterrain', 'grassyterrain', 'mistyterrain'].includes(moveId)) {
			if (difficulty !== 'competitive') return 6;
			return attacker.item === 'terrainextender' as ID ? 9 : 8;
		}

		// --- COUNTER / MIRROR COAT ---
		if (moveId === 'counter' || moveId === 'mirrorcoat') {
			if (difficulty === 'game-accurate') return 6;
			const relevantCategory: 'Physical' | 'Special' = moveId === 'counter' ? 'Physical' : 'Special';
			const canOHKO = this.aiMaxOpponentDamage(defender, attacker) >= attacker.hp;
			const hasSturdy = attacker.ability === 'sturdy' as ID;
			const hasSash = attacker.item === 'focussash' as ID;
			if (canOHKO && !hasSturdy && !hasSash) return -14; // +6 base - 20 penalty
			let score = 6;
			const targetOnlyHasRelevantSplit = (
				this.aiHasMoveCategory(defender, relevantCategory) &&
				!this.aiHasMoveCategory(defender, relevantCategory === 'Physical' ? 'Special' : 'Physical')
			);
			...
			if (defender.moveSlots.some(s => this.dex.moves.get(s.id).category === 'Status') && Math.random() < 0.25) score -= 1;
			return score;
		}
		...
		if (move.boosts) {
			...
			// Pure speed boost → different logic
			if (speBoost > 0 && offBoost === 0 && defBoost === 0) {
				return faster ? -20 : 7;
			}
			...
			// Shell Smash special case
			if (moveId === 'shellsmash') {
				if (difficulty === 'game-accurate') return 6;
				if ((attacker.boosts.atk ?? 0) >= 1) return -20;
				if (difficulty === 'smart') return canOHKO ? -20 : 6;
				if (canOHKO) return -20;
				let score = 6;
				if (this.aiIsIncapacitated(defender)) score += 3;
				// After Smash, def/spd drop by 1 stage (×2/3 defense = ×1.5 incoming damage).
				// If AI has White Herb, def drops are cancelled.
				const hasWhiteHerb = attacker.item === 'whiteherb' as ID;
				const postSmashMult = hasWhiteHerb ? 1.0 : 1.5;
				const postSmashDamage = this.aiMaxOpponentDamage(defender, attacker) * postSmashMult;
				score += postSmashDamage < attacker.hp ? 2 : -2;
				return score;
			}

			// Belly Drum special case: incapacitated check must come before OHKO check
			if (moveId === 'bellydrum') {
				if (difficulty === 'game-accurate') return 6;
				if (this.aiIsIncapacitated(defender)) return 9;
				// After Belly Drum, AI is at 50% HP (Sitrus Berry fires, bringing to ~75%)
				const hasSitrus = attacker.item === 'sitrusberry' as ID;
				const hpAfterDrum = hasSitrus ? attacker.maxhp * 0.75 : attacker.hp * 0.5;
				if (this.aiMaxOpponentDamage(defender, attacker) >= hpAfterDrum) return 4;
				return 8;
			}
			...
			if (defenderHasUnaware && !unawareExceptions.includes(moveId)) return -20;

			const isMixed = offBoost > 0 && defBoost > 0;
			let treatAsOffensive: boolean;
			if (isMixed) {
				// Bulk Up / Coil (physical mixed) → defensive if player only has physical, else offensive
				// Calm Mind / Quiver Dance (special mixed) → defensive if player only has special, else offensive
				if ((boosts.atk ?? 0) > 0 && (boosts.spa ?? 0) === 0) {
					treatAsOffensive = !(this.aiHasMoveCategory(defender, 'Physical') && !this.aiHasMoveCategory(defender, 'Special'));
				} else {
					treatAsOffensive = !(this.aiHasMoveCategory(defender, 'Special') && !this.aiHasMoveCategory(defender, 'Physical'));
				}
			} else {
			...
			if (treatAsOffensive) {
				...
				// Nasty Plot / Tail Glow / Work Up style: extra checks for PURE SpA boosts only.
				// Mixed moves (Calm Mind, Quiver Dance) don't get these — they're treated as
				// offensive/defensive hybrids and the above +3/-5 is sufficient.
				if (!isMixed && (boosts.spa ?? 0) > 0) {
					const can3HKO = this.aiMaxOpponentDamage(defender, attacker) * 3 >= attacker.hp;
					if (!this.aiIsIncapacitated(defender) && !can3HKO) {
						score += 1;
						if (faster) score += 1;
					}
				...
				if (Math.random() < 0.95) {
					if (this.aiIsIncapacitated(defender)) score += 2;
